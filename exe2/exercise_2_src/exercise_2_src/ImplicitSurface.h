...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		Eigen::Vector3d shift_x = _x - m_center;
		double retval = pow(shift_x[0], 2) + pow(shift_x[1], 2) + pow(shift_x[2], 2) - pow(m_radius, 2);
		return retval;
		// return 0.0;
	}
...
public:
	...
	double Eval(const Eigen::Vector3d & _x)
	{
		...
		Eigen::Vector3d shift_x = _x - m_center;
		double retval = pow(pow(shift_x[0], 2) + pow(shift_x[1], 2) + pow(shift_x[2], 2) + pow(m_radius, 2) - pow(m_a, 2), 2) - \
						4 * pow(m_radius, 2) * (pow(shift_x[0], 2) + pow(shift_x[1], 2));
		return retval;
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		Vector3d b(m_coefficents[m_numCenters], m_coefficents[m_numCenters+1], m_coefficents[m_numCenters+2]);
		result += b.dot(_x);
	...
	void BuildSystem()
	{
		...
		for(int i = 0; i<2*m_numCenters; i++){
			for(int j = 0; j<m_numCenters+4; j++){
				if(j<m_numCenters) A(i, j) = phi(i,j);
				else if(j=m_numCenters+3) A(i, j) = 1;
				else A(i, j) = m_funcSamp.m_pos[i][j-m_numCenters];
			}
